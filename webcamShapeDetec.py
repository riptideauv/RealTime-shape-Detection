...
while 1:
    ret,frame=cap.read()
    frame=cv2.flip(frame,1)   #goruntuyu ceviriyor
    ...
    for cnt in contours:
        ...
        epsilon=0.2*cv2.arcLength(cnt,True)  #True seklilin kapalı oldugunu gosteriyor
        approx=cv2.approxPolyDP(cnt,epsilon,True)
        x=approx.ravel()[0]
        y=approx.ravel()[1]
        
        if area > 400: #400den kucuk alanlarda sekil arama
            cv2.drawContours(frame,[approx],0,(0,0,0),5)
            if len(approx)==3:
                cv2.putText(frame,"Triangle",(x,y),font,1,(0,0,0))
                
            elif len(approx) ==4:
                cv2.putText(frame,"Recangle",(x,y),font,1,(0,0,0))
